...
import com.zb.mmseg.core.example.Complex;

/**
 * @author zxc Sep 3, 2014 2:44:43 PM
 */
public class ComplexSegTest {
    ...
    /*
     * public void testSeg() { String txt = ""; txt = "各人发表关于受一股来自中西伯利亚的强冷空气影响"; ComplexSeg.setShowChunk(true);
     * ComplexSeg seg = new ComplexSeg(new Dictionary("dic")); //sogou Sentence sen = new Sentence(txt.toCharArray(),
     * 0); System.out.println(); while(!sen.isFinish()) { Chunk chunk = seg.seg(sen);
    ...
    public void testUnitEffect() throws IOException {
        String words = segW.segWords("2014年底发了资金吗", "|");
        Assert.assertEquals("2014|年|底|发|了|资金|吗", words);
    }
