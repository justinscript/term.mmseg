...
public class MaxWordSeg extends ComplexSeg {
    ...
    public MaxWordSeg(MMSegDictionary dic) {
        super(dic);
    }
    ...
    public Chunk seg(Sentence sen) {
        ...
        if (chunk != null) {
            List<MMSegWord> cks = new ArrayList<MMSegWord>();
            for (int i = 0; i < chunk.getCount(); i++) {
                MMSegWord word = chunk.words[i];
                ...
                if (word.getLength() < 3) {
                    ...
                    for (; offset < wordEnd - 1; offset++) {
                        ...
                        if (idx > -1) {
                            cks.add(new MMSegWord(chs, senStartOffset, offset, 2));
                            end = offset + 2;
                            n++;
                        } else if (offset >= end) { // 有单字
                            cks.add(new MMSegWord(chs, senStartOffset, offset, 1));
                            end = offset + 1;
            ...
            chunk.words = cks.toArray(new MMSegWord[cks.size()]);
