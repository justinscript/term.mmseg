...
import com.zb.mmseg.core.rule.LargestAvgLenRule;
import com.zb.mmseg.core.rule.LargestSumDegreeFreedomRule;
import com.zb.mmseg.core.rule.MaxMatchRule;
import com.zb.mmseg.core.rule.Rule;
import com.zb.mmseg.core.rule.SmallestVarianceRule;
...
public class ComplexSeg extends Seg {
    ...
    public ComplexSeg(MMSegDictionary dic) {
        super(dic);
    ...
    public Chunk seg(Sentence sen) {
        ...
        if (!sen.isFinish()) { // sen.getOffset() < chs.length
            ...
            /*
             * 遍历所有不同词长,还不是从最大到0(w[0]=maxLen(chs, offsets[0]); w[0]>=0; w[0]--) 可以减少一部分多余的查找.
             */
    ...
    private Chunk createChunk(Sentence sen, char[] chs, int[] tailLen, int[] offsets, CharNode[] cns/* , char[][] cks */) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (offsets[i] < chs.length) {
                ck.words[i] = new MMSegWord(chs, sen.getStartOffset(), offsets[i], tailLen[i] + 1);// new Word(cks[i],
                                                                                              // sen.getStartOffset()+offsets[i]);
