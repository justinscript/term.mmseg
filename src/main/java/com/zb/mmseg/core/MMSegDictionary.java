...
public class MMSegDictionary {
    ...
    private static final Logger                                   log           = Logger.getLogger(MMSegDictionary.class.getName());
    ...
    private static final ConcurrentHashMap<File, MMSegDictionary> dics          = new ConcurrentHashMap<File, MMSegDictionary>();
     ...
     * @see #getDefalutPath()
     */
    public static MMSegDictionary getInstance() {
        File path;
        try {
            path = getDefalutPath();
        ...
        return getInstance(StringUtils.EMPTY);
    }

    /**
     * @param path 词典的目录
     */
    public static MMSegDictionary getInstance(String path) {
     ...
     * @param path 词典的目录
     */
    public static MMSegDictionary getInstance(File path) {
        log.info("try to load dir=" + path);
        File normalizeDir = normalizeFile(path);
        MMSegDictionary dic = dics.get(normalizeDir);
        if (dic == null) {
            dic = new MMSegDictionary(normalizeDir);
    ...
    /**
     * @see MMSegDictionary#clear(File)
     */
    public static MMSegDictionary clear(String path) {
     ...
     * @return 没有返回 null
     */
    public static MMSegDictionary clear(File path) {
        File normalizeDir = normalizeFile(path);
     ...
     * 词典的目录
     */
    private MMSegDictionary(File path) {
        init(path);
    }
    ...
    private Map<Character, Object> loadUnit(File path) throws IOException {
        ...
        if (unitFile.exists()) {
            ...
            fin = MMSegDictionary.class.getResourceAsStream("/data/units.dic");
            unitFile = new File(MMSegDictionary.class.getResource("/data/units.dic").getFile());
        }
        ...
        int lineNum = load(fin, new FileLoading() {
            ...
            public void row(String line, int n) {
                ...
                unit.put(line.charAt(0), MMSegDictionary.class);
            }
        });
     ...
     * 加载 wordsXXX.dic 文件类。
     * 
     * @author zb 2009-10-15 下午02:12:55
     */
     ...
     */
    public static File getDefalutPath() throws Exception {
        if (defalutPath == null) {
            String defPath = System.getProperty("mmseg.dic.path");
            log.info("getDefalutPath look up in mmseg.dic.path=" + defPath);
            if (defPath == null) {
                URL url = MMSegDictionary.class.getClassLoader().getResource("data/words.dic");
                if (url != null) {
                    defPath = url.getFile();
                    log.info("getDefalutPath look up in classpath=" + defPath);
                } else {
                    defPath = System.getProperty("user.dir") + "/data";
                    log.info("getDefalutPath look up in user.dir=" + defPath);
                }
            }

            defalutPath = new File(defPath);
            if (!defalutPath.exists()) {
                log.warning("getDefalutPath defalut dic path=" + defalutPath + " not exist");
                log.warning("getDefalutPath load defalutPath by jar file!");

                // String jarPath = MMSegDictionary.class.getProtectionDomain().getCodeSource().getLocation().getFile();
                String mmsegJarPath = defPath;
                log.info("getDefalutPath jarPath :" + mmsegJarPath);
                mmsegJarPath = StringUtils.remove(mmsegJarPath, ".jar!/data/words.dic");
                File workDir = new File(mmsegJarPath);
                workDir.delete();
                workDir.mkdirs();
                if (!workDir.isDirectory()) {
                    log.info("getDefalutPath Mkdirs failed to create " + workDir);
                    throw new IOException("getDefalutPath Mkdirs failed to create " + workDir);
                }
                log.info("getDefalutPath workDir :" + workDir.getPath());
                unJar(new JarFile(StringUtils.remove(mmsegJarPath, "file:") + ".jar"), workDir);
                defalutPath = new File(mmsegJarPath + "/data");
            }
        }
        return defalutPath;
    }
