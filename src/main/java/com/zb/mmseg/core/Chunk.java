...
/**
 * 它是MMSeg分词算法中一个关键的概念。Chunk中包含依据上下文分出的一组词和相关的属性，包括长度(Length)、平均长度(Average Length)、标准差的平方(Variance)和自由语素度(Degree Of
 * Morphemic Freedom)。
 * 
...
public class Chunk {
    ...
    MMSegWord[]         words     = new MMSegWord[3];
    ...
    public MMSegWord[] getWords() {
        return words;
    }

    public void setWords(MMSegWord[] words) {
        this.words = words;
        count = words.length;
    }
}
