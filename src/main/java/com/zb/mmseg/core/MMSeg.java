...
public class MMSeg {
    ...
    private Queue<MMSegWord>    bufWord;                             // word 缓存, 因为有 chunk 分析三个以上.
    ...
    public void reset(Reader input) {
        ...
        bufWord = new LinkedList<MMSegWord>();
    ...
    public MMSegWord next() throws IOException {
        // 先从缓存中取
        MMSegWord word = bufWord.poll();
        ;
        if (word == null) {
            bufSentence.setLength(0);
            ...
            while (read && (data = readNext()) != -1) {
                ...
                String wordType = MMSegWord.TYPE_WORD;
                switch (type) {
                    ...
                    case Character.MODIFIER_LETTER:
                        /*
                         * 1. 0x410-0x44f -> А-я //俄文 2. 0x391-0x3a9 -> Α-Ω //希腊大写 3. 0x3b1-0x3c9 -> α-ω //希腊小写
                         */
                        data = toAscii(data);
                        ...
                        wordType = MMSegWord.TYPE_LETTER;
                        ...
                        switch (nl) {
                            case EN:
                                ...
                                if (rcad.hasDigit()) {
                                    wordType = MMSegWord.TYPE_LETTER_OR_DIGIT;
                                }
                                // only english
                    ...
                    case Character.DECIMAL_DIGIT_NUMBER:
                        ...
                        wordType = MMSegWord.TYPE_DIGIT;
                        int d = readNext();
                        if (d > -1) {
                            if (seg.isUnit(d)) { // 单位,如时间
                                bufWord.add(createWord(bufSentence, startIdx(bufSentence) - 1, MMSegWord.TYPE_DIGIT)); // 先把数字添加(独立)

                                bufSentence.setLength(0);

                                bufSentence.appendCodePoint(d);
                                wordType = MMSegWord.TYPE_WORD; // 单位是 word
                            } else { // 后面可能是字母和数字
                                pushBack(d);
                                if (readChars(bufSentence, new ReadCharByAsciiOrDigit()) > 0) { // 如果有字母或数字都会连在一起.
                                    wordType = MMSegWord.TYPE_DIGIT_OR_LETTER;
                                }
                            }
                        }
                    ...
                    case Character.LETTER_NUMBER:
                        ...
                        for (int i = 0; i < bufSentence.length(); i++) {
                            bufWord.add(new MMSegWord(new char[] { bufSentence.charAt(i) }, startIdx++,
                                                 MMSegWord.TYPE_LETTER_NUMBER));
                        }
                    ...
                    case Character.OTHER_NUMBER:
                        ...
                        bufWord.add(createWord(bufSentence, MMSegWord.TYPE_OTHER_NUMBER));
                        bufSentence.setLength(0);
                        break;
                    default:
                        // 其它认为无效字符
    ...
    private MMSegWord createWord(StringBuilder bufSentence, String type) {
        return new MMSegWord(toChars(bufSentence), startIdx(bufSentence), type);
    }

    private MMSegWord createWord(StringBuilder bufSentence, int startIdx, String type) {
        return new MMSegWord(toChars(bufSentence), startIdx, type);
    }
